...
const scheduleRender = () => {
  ...
  renderTimeout = setTimeout(() => {
    ...
    else if (state.currentPage === 'transfers') renderTransfersPage();
    else if (state.currentPage === 'supply-orders') renderSupplyOrdersPage();
...
const parseNum = (v) => {
  ...
  if (typeof v === 'number') { if (!isFinite(v)) return 0; return round2(v); }
...
const fmt = (n) => {
  ...
  return Number(n).toLocaleString(APP_CONFIG.LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
};
...
const computeCheck = (chk) => {
  if (!chk || typeof chk !== 'object') {
    return { checkAmount:0, supply:0, pct13:0, pct1:0, vat:0, taxablePurchase:0, supplyTotal:0, actualTotal:0, expensesTotal:0, taxProfit:0, tax225:0, remaining225:0, buyer14:0, buyerNet:0, actualProfit:0, moazPct:50, amrPct:50, moazProfitShare:0, amrProfitShare:0, moazTotal:0, amrTotal:0, supplyDifference:0, supplyMatch:false };
  }
  ...
  return {
    checkAmount, supply, pct13, pct1, vat, taxablePurchase, supplyTotal, actualTotal, expensesTotal, taxProfit, tax225,
    remaining225, buyer14, buyerNet, actualProfit, moazPct, amrPct, moazProfitShare, amrProfitShare,
    moazTotal: round2(moazProfitShare + remaining225), amrTotal: amrProfitShare,
    supplyDifference: Math.abs(supply - supplyTotal), supplyMatch: supply > 0 && supplyTotal > 0 && Math.abs(supply - supplyTotal) < 0.01
  };
};
...
const getCurrentSupplyOrder = () => state.supplyOrders.find(o => o.id === state.currentSupplyOrderId) || null;
...
const linkSupplyOrderToCheck = (supplyOrderId, checkId, amount) => {
  ...
  order.linkedChecks.push({ checkId: checkId, amount: amountToAdd, date: new Date().toISOString().split('T')[0] });
...
const createNewCheck = () => {
  ...
  const chk = createEntity({
    ...
    checkNumber: '', checkAmount: '', checkDate: '', received: false, collectionDate: '',
...
const bindMoneyInput = (id, field) => {
  ...
  if (!input) return;
  input.addEventListener('focus', () => { const raw = parseNum(input.value); if (raw > 0) input.value = raw; });
  input.addEventListener('blur', () => {
    const chk = getCurrentCheck();
    if (!chk || isLocked(chk)) return;
    const raw = parseNum(input.value);
    chk[field] = raw > 0 ? raw : '';
    if (raw > 0) input.value = fmt(raw); else input.value = '';
    saveAndRender();
  });
};

const updateField = (field, value) => { const chk = getCurrentCheck(); if (!chk || isLocked(chk)) return; chk[field] = value; saveAndRender(); };
const onMoneyFocus = (input) => { const raw = parseNum(input.value); input.value = raw > 0 ? raw : ''; };
const onMoneyBlur = (input, idx, field) => {
  const raw = parseNum(input.value);
  input.value = raw > 0 ? fmt(raw) : '';
  if (idx === undefined || idx === null || isNaN(idx)) return;
  if (field === 'supplyValue' || field === 'actualPurchase') updateSupplyOrder(idx, field, raw > 0 ? raw : '');
  else if (field === 'value') updateExpense(idx, field, raw > 0 ? raw : '');
};
...
const renderSupplyOrdersPage = () => {
  document.querySelectorAll('.nav-btn').forEach(btn => btn.classList.toggle('active', btn.dataset.page === 'supply-orders'));
  ...
  document.querySelectorAll('.page-container').forEach(container => container.classList.toggle('active', container.id === 'page-supply-orders'));
...
const renderSupplyOrdersSidebar = () => {
  ...
  if (sf) {
    const cf = sf.value;
    sf.innerHTML = '<option value="">— كل الحالات —</option>' + Object.values(SUPPLY_ORDER_STATUS).map(s => `<option value="${s.value}" ${s.value === cf ? 'selected' : ''}>${s.label}</option>`).join('');
  }
  ...
  list.innerHTML = filtered.map(o => {
    ...
    return `
      <div class="check-item ${o.id === state.currentSupplyOrderId ? 'active' : ''}" onclick="selectSupplyOrder('${o.id}')">
        <div class="ci-top"><span class="ci-num">#${esc(o.orderNumber || '—')}</span><span class="ci-status ${status.class}">${status.label}</span></div>
        <div class="ci-meta">${o.date || '—'}</div>
        <div class="ci-amount">${supplyValue > 0 ? fmt(supplyValue) + ' ' + APP_CONFIG.CURRENCY : '—'}${coveredAmount > 0 ? '<br><small style="color:#10b981;">مغطى: ' + fmt(coveredAmount) + '</small>' : ''}</div>
      </div>
    `;
  }).join('');
  const set = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
  set('so-count', orders.length);
  set('so-total-value', fmt(orders.reduce((s, o) => s + parseNum(o.supplyValue), 0)));
  set('so-total-covered', fmt(orders.reduce((s, o) => s + parseNum(o.coveredAmount), 0)));
  set('so-pending', orders.filter(o => o.status === 'pending').length);
};
...
const renderSupplyOrdersMain = () => {
  ...
  form.innerHTML = `
    <div class="edit-section"><h3>بيانات أمر التوريد</h3><div class="field-grid">
      <div class="field"><label>رقم الأمر</label><input type="text" id="so-order-number" value="${esc(order.orderNumber)}" onchange="updateSupplyOrderField('orderNumber', this.value)"></div>
      <div class="field"><label>تاريخ الأمر</label><input type="date" id="so-date" value="${order.date || ''}" onchange="updateSupplyOrderField('date', this.value)"></div>
      <div class="field"><label>قيمة التوريد</label><input type="text" id="so-supply-value" value="${supplyValue > 0 ? fmt(supplyValue) : ''}" inputmode="decimal" placeholder="0.00" class="num money-input" onfocus="onMoneyFocus(this)" onblur="onSupplyOrderMoneyBlur(this, 'supplyValue')"></div>
      <div class="field"><label>سعر الشراء الفعلي</label><input type="text" id="so-purchase-price" value="${purchasePrice > 0 ? fmt(purchasePrice) : ''}" inputmode="decimal" placeholder="0.00" class="num money-input" onfocus="onMoneyFocus(this)" onblur="onSupplyOrderMoneyBlur(this, 'purchasePrice')"></div>
      <div class="field"><label>الحالة</label><div class="computed ${status.class === 'status-covered' ? 'profit-val' : status.class === 'status-partial' ? 'highlight' : ''}">${status.label}</div></div>
      <div class="field"><label>المبلغ المغطى</label><div class="computed">${fmt(coveredAmount)}</div></div>
      <div class="field"><label>المتبقي غير مغطى</label><div class="computed ${remaining > 0 ? 'danger-val' : 'profit-val'}">${fmt(remaining)}</div></div>
      <div class="field" style="grid-column: span 2;"><label>ملاحظات</label><input type="text" id="so-notes" value="${esc(order.notes || '')}" onchange="updateSupplyOrderField('notes', this.value)" placeholder="ملاحظات اختيارية"></div>
    </div></div>
    <div class="edit-section"><h3>الشيكات المرتبطة</h3>
      ${(order.linkedChecks && order.linkedChecks.length > 0) ? `
        <table class="edit-table"><thead><tr><th>م</th><th>رقم الشيك</th><th>المبلغ المغطى</th><th>التاريخ</th></tr></thead><tbody>
          ${order.linkedChecks.map((lc, i) => {
            const check = state.checks.find(c => c.id === lc.checkId);
            return `<tr><td>${i + 1}</td><td>${check ? '#' + esc(check.checkNumber || '—') : 'شيك محذوف'}</td><td class="num">${fmt(lc.amount)}</td><td>${lc.date || '—'}</td></tr>`;
          }).join('')}
        </tbody><tfoot><tr class="total-row"><td colspan="2">إجمالي المغطى</td><td class="num">${fmt(coveredAmount)}</td><td></td></tr></tfoot></table>
      ` : '<p style="color:#64748b;text-align:center;padding:15px;">لا توجد شيكات مرتبطة بهذا الأمر</p>'}
    </div>
    <div class="edit-section"><h3>ملخص مالي</h3><div class="field-grid">
      <div class="field"><label>قيمة التوريد</label><div class="computed big">${fmt(supplyValue)}</div></div>
      <div class="field"><label>سعر الشراء</label><div class="computed">${fmt(purchasePrice)}</div></div>
      <div class="field"><label>الفرق (ربح/خسارة)</label><div class="computed ${supplyValue - purchasePrice >= 0 ? 'profit-val' : 'danger-val'}">${fmt(supplyValue - purchasePrice)}</div></div>
    </div></div>
  `;
  const bindFilter = (id, fn) => { const el = document.getElementById(id); if (el) el.addEventListener('change', fn); };
...
const renderSupplyOrdersTable = () => {
  const container = document.getElementById('so-table-container');
  if (!container) return;
  const orders = state.supplyOrders || [];
  const monthFilter = document.getElementById('so-month-filter')?.value || '';
  const statusFilter = document.getElementById('so-status-filter')?.value || '';
  const searchText = document.getElementById('search-supply-orders')?.value?.toLowerCase() || '';
  let filtered = orders.filter(o => {
    if (monthFilter && !o.date?.startsWith(monthFilter)) return false;
    if (statusFilter && o.status !== statusFilter) return false;
    if (searchText) {
      const searchContent = `${o.orderNumber || ''} ${o.supplyValue || ''} ${o.purchasePrice || ''} ${o.notes || ''}`.toLowerCase();
  ...
  filtered.sort((a, b) => (b.date || '').localeCompare(a.date || '') || (b.createdAt || 0) - (a.createdAt || 0));
  if (filtered.length === 0) {
    container.innerHTML = '<div class="empty-state"><h3>لا توجد أوامر توريد</h3><p>اضغط "➕ إضافة أمر توريد" للبدء</p></div>';
    return;
  }
  container.innerHTML = `
    <table class="data-table">
      <thead><tr><th>رقم الأمر</th><th>التاريخ</th><th>قيمة التوريد</th><th>سعر الشراء</th><th>المغطى</th><th>المتبقي</th><th>الحالة</th><th>إجراء</th></tr></thead>
      <tbody>
        ${filtered.map(o => {
          const status = SUPPLY_ORDER_STATUS[o.status?.toUpperCase()] || SUPPLY_ORDER_STATUS.PENDING;
          const supplyValue = parseNum(o.supplyValue);
          const purchasePrice = parseNum(o.purchasePrice);
          const coveredAmount = parseNum(o.coveredAmount);
          const remaining = Math.max(0, supplyValue - coveredAmount);
          const isSelected = o.id === state.currentSupplyOrderId;
          return `<tr class="${isSelected ? 'selected-row' : ''}" style="cursor:pointer;" onclick="selectSupplyOrder('${o.id}')">
            <td><strong>#${esc(o.orderNumber || '—')}</strong></td><td>${o.date || '—'}</td><td class="num">${supplyValue > 0 ? fmt(supplyValue) : '—'}</td>
            <td class="num">${purchasePrice > 0 ? fmt(purchasePrice) : '—'}</td><td class="num">${coveredAmount > 0 ? fmt(coveredAmount) : '—'}</td>
            <td class="num">${remaining > 0 ? fmt(remaining) : '—'}</td><td><span class="badge ${status.class}">${status.label}</span></td>
            <td><button onclick="event.stopPropagation(); selectSupplyOrder('${o.id}')">عرض</button></td>
          </tr>`;
        }).join('')}
      </tbody>
    </table>
...
const calcTransferCounters = () => {
  ...
  const regularTransfers = safeArray(transfers).filter(t => !safeArray(t.allocations).some(a => a.type === 'profit'));
...
const renderTransfersPage = () => {
  ...
  const expenses = safeArray(state.transfers.purchases);
  ...
  if (balanceStatusEl) {
    if (balance >= 0) { balanceStatusEl.textContent = 'لنا عندهم ✅'; balanceStatusEl.style.color = '#059669'; }
    else { balanceStatusEl.textContent = 'علينا لهم ⚠️'; balanceStatusEl.style.color = '#dc2626'; }
  }
...
const renderTransfersList = () => {
  ...
  const arr = safeArray(transfers).sort((a, b) => (b.date || '').localeCompare(a.date || '') || (b.createdAt || 0) - (a.createdAt || 0));
...
const renderPurchasesList = () => {
  ...
  if (!list) return;
  const purchases = safeArray(state.transfers.purchases).sort((a, b) => (b.date || '').localeCompare(a.date || '') || (b.createdAt || 0) - (a.createdAt || 0));
  if (purchases.length === 0) { list.innerHTML = '<div class="empty-list">لا توجد مصروفات</div>'; return; }
...
const createNewTransfer = () => { editTransferDialog(createEntity({ date: new Date().toISOString().split('T')[0], amount: '', type: 'تحويل بنكي', notes: '', allocations: [] }), true); };
const editTransfer = (id) => { const t = safeArray(state.transfers.transactions).find(x => x.id === id); if (t) editTransferDialog(t, false); };
const deleteTransfer = (id) => { deleteWithConfirm('هل أنت متأكد من حذف هذا التحويل؟', () => { state.transfers.transactions = safeArray(state.transfers.transactions).filter(t => t.id !== id); state.transfers.purchases = safeArray(state.transfers.purchases).filter(p => p.transferId !== id); }, renderTransfersPage); };
const createNewPurchase = () => { editPurchaseDialog(createEntity({ date: new Date().toISOString().split('T')[0], amount: '', reference: '', notes: '', status: 'pending' }), true); };
const editPurchase = (id) => { const p = safeArray(state.transfers.purchases).find(x => x.id === id); if (p) editPurchaseDialog(p, false); };
const deletePurchase = (id) => { const purchase = safeArray(state.transfers.purchases).find(p => p.id === id); deleteWithConfirm('هل أنت متأكد من حذف هذه المصروفات؟', () => { state.transfers.purchases = safeArray(state.transfers.purchases).filter(p => p.id !== id); if (purchase && purchase.transferId) { const transfer = safeArray(state.transfers.transactions).find(t => t.id === purchase.transferId); if (transfer && Array.isArray(transfer.allocations)) transfer.allocations = transfer.allocations.filter(a => a.type !== 'purchase'); } }, renderTransfersPage); };
...
const switchTransferTab = (tabName) => {
  document.querySelectorAll('#page-transfers .tab').forEach(t => {
    ...
    if (isActive) { t.style.background = '#f1f5f9'; t.style.color = '#1e2937'; }
    else { t.style.background = 'transparent'; t.style.color = '#64748b'; }
  });
...
const editTransferDialog = (transfer, isNew) => {
  ...
  const content = `<div class="form-group"><label>التاريخ</label><input type="date" id="t-date" value="${transfer.date}"></div><div class="form-group"><label>المبلغ</label><input type="text" id="t-amount" value="${transfer.amount > 0 ? fmt(transfer.amount) : ''}" inputmode="decimal" placeholder="0.00" class="num-input"></div><div class="form-group"><label>نوع التحويل</label><select id="t-type">${ttHTML}</select></div><div class="form-group"><label>ملاحظات</label><textarea id="t-notes" rows="3">${esc(transfer.notes)}</textarea></div><div class="form-group allocations-box"><label>التقسيم (اختياري)</label><div id="allocations-list"></div><button type="button" class="btn-add-allocation" onclick="addAllocation()">➕ إضافة تقسيم</button></div>`;
...
const renderAllocations = () => {
  ...
  list.innerHTML = allocs.map((a, i) => `<div class="allocation-row"><select class="alloc-type" onchange="updateAllocationType(${i}, this.value)">${Object.values(ALLOCATION_TYPES).map(at => `<option value="${at.value}" ${(a.type||'purchase') === at.value ? 'selected' : ''}>${at.icon} ${at.label}</option>`).join('')}</select><input type="text" class="alloc-amount num-input" value="${a.amount > 0 ? fmt(a.amount) : ''}" inputmode="decimal" placeholder="0.00" onchange="updateAllocationAmount(${i}, this.value)"><button type="button" class="btn-remove" onclick="removeAllocation(${i})">🗑</button></div>`).join('');
};
const addAllocation = () => { if (!currentTransferData) return; currentTransferData.allocations = safeArray(currentTransferData.allocations); currentTransferData.allocations.push({ type: 'purchase', amount: '' }); renderAllocations(); };
const removeAllocation = (idx) => { if (!currentTransferData || !Array.isArray(currentTransferData.allocations)) return; currentTransferData.allocations.splice(idx, 1); renderAllocations(); };
const updateAllocationType = (idx, value) => { if (!currentTransferData || !Array.isArray(currentTransferData.allocations) || !currentTransferData.allocations[idx]) return; currentTransferData.allocations[idx].type = value; };
const updateAllocationAmount = (idx, value) => { if (!currentTransferData || !Array.isArray(currentTransferData.allocations) || !currentTransferData.allocations[idx]) return; currentTransferData.allocations[idx].amount = parseNum(value); };
...
const saveTransfer = (id, isNew) => {
  ...
  if (isNew) state.transfers.transactions.push(transfer);
  else { const idx = state.transfers.transactions.findIndex(t => t.id === id); if (idx !== -1) { transfer.createdAt = state.transfers.transactions[idx].createdAt; state.transfers.transactions[idx] = transfer; } }
  ...
  if (totalPurchaseAmount > 0) {
    if (firstExisting) { firstExisting.amount = totalPurchaseAmount; firstExisting.date = date; }
    else state.transfers.purchases.push({ id: uid(), transferId: id, amount: totalPurchaseAmount, date: date, reference: '', notes: `من تحويل ${date}`, status: 'pending', createdAt: Date.now() });
  } else {
    if (firstExisting) firstExisting.amount = 0;
    else state.transfers.purchases = safeArray(state.transfers.purchases).filter(p => p.transferId !== id);
  }
  saveState(); closeModal(); renderTransfersPage();
};

const editPurchaseDialog = (purchase, isNew) => {
  const content = `<div class="form-group"><label>التاريخ</label><input type="date" id="p-date" value="${purchase.date}"></div><div class="form-group"><label>المبلغ</label><input type="text" id="p-amount" value="${purchase.amount > 0 ? fmt(purchase.amount) : ''}" inputmode="decimal" placeholder="0.00" class="num-input"></div><div class="form-group"><label>رقم المرجع (اختياري)</label><input type="text" id="p-reference" value="${esc(purchase.reference)}" placeholder="INV-001"></div><div class="form-group"><label>ملاحظات</label><textarea id="p-notes" rows="3">${esc(purchase.notes)}</textarea></div>`;
  showModal(isNew ? '➕ مصروف جديد' : '✏️ تعديل مصروف', content, () => savePurchase(purchase.id, isNew), { maxWidth: '500px' });
};

const savePurchase = (id, isNew) => {
  const existing = safeArray(state.transfers.purchases).find(p => p.id === id);
  const purchase = { id, date: document.getElementById('p-date').value, amount: parseNum(document.getElementById('p-amount').value), reference: document.getElementById('p-reference').value, notes: document.getElementById('p-notes').value, status: existing ? existing.status : 'pending', createdAt: existing ? existing.createdAt : Date.now(), transferId: existing ? existing.transferId : undefined };
  if (isNew) state.transfers.purchases.push(purchase);
  else { const idx = state.transfers.purchases.findIndex(p => p.id === id); if (idx !== -1) state.transfers.purchases[idx] = purchase; }
  saveState(); closeModal(); renderTransfersPage();
};

const closeModal = () => { const modal = document.querySelector('.modal-overlay'); if (modal) modal.remove(); currentTransferData = null; };

// ============================================
// 🆕 نافذة إضافة مصروف ذكية (Checkboxes)
// ============================================
const renderBuyerExpenseDialog = () => {
  ...
  const content = `
    ...
    <div class="form-group"><label>نوع المصروف</label><select id="exp-type" onchange="toggleExpenseFields()">
    ...
    <div id="exp-purchase-fields" style="display:block;">
      <div class="form-group"><label style="font-weight:700; margin-bottom:8px; display:block;">✅ اختر أوامر التوريد التي تم شراؤها:</label>
        <div style="max-height:200px; overflow-y:auto; border:1px solid #cbd5e1; border-radius:5px; padding:8px; background:#fff;">${ordersCheckboxes}</div>
      </div>
      <div class="form-group"><label>إجمالي سعر الشراء (محسوب تلقائياً)</label><input type="text" id="exp-purchase-total" value="0.00" readonly class="num-input" style="background:#f1f5f9;"></div>
      <div class="form-group"><label>مصاريف النقل الإضافية لهذه الأوامر (اختياري)</label><input type="text" id="exp-transport-amount" value="" inputmode="decimal" placeholder="0.00" class="num-input" onchange="calculateExpenseTotal()"></div>
    </div>
    <div id="exp-transport-fields" style="display:none;">
      <div class="form-group"><label style="font-weight:700; margin-bottom:8px; display:block;"> أوامر التوريد المرتبطة بهذا النقل (اختياري):</label>
        <div style="max-height:150px; overflow-y:auto; border:1px solid #cbd5e1; border-radius:5px; padding:8px; background:#fff;">${allOrdersCheckboxes}</div>
      </div>
      <div class="form-group"><label>قيمة النقل</label><input type="text" id="exp-transport-val" value="" inputmode="decimal" placeholder="0.00" class="num-input" onchange="calculateExpenseTotal()"></div>
    </div>
    <div id="exp-other-fields" style="display:none;">
      <div class="form-group"><label>وصف المصروف</label><input type="text" id="exp-other-desc" placeholder="مثال: جمارك، تخزين، عمالة"></div>
      <div class="form-group"><label>المبلغ</label><input type="text" id="exp-other-amount" value="" inputmode="decimal" placeholder="0.00" class="num-input" onchange="calculateExpenseTotal()"></div>
    </div>
...
const calculateExpenseTotal = () => {
  ...
  if (type === 'purchase') {
    ...
    let purchaseTotal = 0;
    checkboxes.forEach(cb => { purchaseTotal += parseNum(cb.dataset.price); });
...
const saveBuyerExpense = () => {
  ...
  const type = document.getElementById('exp-type').value;
  let amount = 0, reference = '', notes = '', linkedSupplyOrderIds = [];

  if (type === 'purchase') {
    const checkboxes = document.querySelectorAll('.supply-order-checkbox:checked');
    if (checkboxes.length === 0) { alert('⚠️ يرجى اختيار أمر توريد واحد على الأقل'); return; }
    let purchaseTotal = 0;
    checkboxes.forEach(cb => { linkedSupplyOrderIds.push(cb.value); purchaseTotal += parseNum(cb.dataset.price); });
    const transport = parseNum(document.getElementById('exp-transport-amount').value);
    amount = purchaseTotal + transport;
    reference = 'أوامر: ' + Array.from(checkboxes).map(cb => { const order = state.supplyOrders.find(o => o.id === cb.value); return '#' + (order?.orderNumber || '—'); }).join(', ');
    ...
    reference = 'نقل';
    notes = linkedSupplyOrderIds.length > 0 ? 'مرتبط بـ: ' + linkedSupplyOrderIds.map(id => { const order = state.supplyOrders.find(o => o.id === id); return '#' + (order?.orderNumber || '—'); }).join(', ') : 'نقل عام';
  } else if (type === 'other') {
  ...
  if (amount <= 0) { alert('⚠️ يرجى إدخال مبلغ صحيح أكبر من صفر'); return; }

  const expense = createEntity({ date, amount, type, reference, notes, status: 'completed', linkedSupplyOrderIds });
  ...
  saveState(); closeModal(); renderTransfersPage();
  if (state.currentPage === 'supply-orders') { renderSupplyOrdersSidebar(); renderSupplyOrdersTable(); }
};
...
const exportPDF = async (elementId, filename, btnId) => {
  ...
  try {
    const opt = { margin: [5, 5, 5, 5], filename: filename, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 1.5, useCORS: true, backgroundColor: '#ffffff', logging: false, windowWidth: 1200, scrollX: 0, scrollY: -element.offsetTop, letterRendering: true, allowTaint: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait', compress: true }, pagebreak: { mode: ['avoid-all', 'css', 'legacy'], before: '.rpt-section' } };
    await html2pdf().set(opt).from(element).save();
  } catch(err) { console.error('PDF Error:', err); alert('خطأ في إنشاء PDF: ' + err.message); } 
  finally { if (btn) { btn.textContent = origText; btn.disabled = false; } }
};
const exportCurrentPDF = async () => { const chk = getCurrentCheck(); if (!chk) { alert('لا يوجد شيك محدد'); return; } switchTab('report'); await new Promise(resolve => setTimeout(resolve, 300)); await exportPDF('report-view', `تقرير_شيك_${chk.checkNumber || 'بدون_رقم'}_${chk.workMonth}.pdf`, 'btn-export-pdf'); };
const exportTransfersPDF = async () => { await exportPDF('page-transfers', `تقرير_التحويلات_${new Date().toISOString().slice(0,10)}.pdf`, 'btn-export-transfers-pdf'); };

const buildExcelRows = (chk, c) => {
  const rows = []; const so = safeArray(chk.supplyOrders); const ex = safeArray(chk.expenses); const buyerName = getBuyerDisplayName();
  rows.push(['تقرير تسوية شيك'], ['تقرير رقم:', `CHK-${chk.workMonth}-${chk.checkNumber||'001'}`, '', 'شهر العمل:', workMonthLabel(chk.workMonth), 'رقم الشيك:', chk.checkNumber||'—'], [], ['بيانات الشيك'], ['شهر العمل', workMonthLabel(chk.workMonth), '', 'قيمة التوريد', c.supply, '', 'تم الاستلام', chk.received?'نعم':'لا'], ['رقم الشيك', chk.checkNumber||'—', '', 'قيمة 13%', c.pct13, '', 'تاريخ الصرف', chk.checkDate||'—'], ['مبلغ الشيك', c.checkAmount, '', 'قيمة 1%', c.pct1, '', 'تاريخ التحصيل', chk.collectionDate||'—'], [], ['أوامر التوريد'], ['م', 'رقم الأمر', 'قيمة التوريد', 'الشراء الفعلي']);
  so.forEach((o,i) => rows.push([i+1, o.orderNumber||'', parseNum(o.supplyValue), parseNum(o.actualPurchase)]));
  rows.push(['', 'الإجمالي', c.supplyTotal, c.actualTotal], [], [`حساب ${buyerName}`], ['صافي المستحق', c.buyerNet], [], ['المصاريف'], ['م', 'البند', 'القيمة']);
  ex.forEach((e,i) => rows.push([i+1, e.label||'', parseNum(e.value)]));
  rows.push(['', 'الإجمالي', c.expensesTotal], [], ['ضريبة 22.5%'], ['الربح الضريبي', c.taxProfit], ['ضريبة 22.5%', c.tax225], [], ['الربح الفعلي', c.actualProfit], [], ['ملخص التسوية'], [buyerName, c.buyerNet], [PARTNERS.PARTNER_2, c.amrTotal], [PARTNERS.PARTNER_1, c.moazTotal], ['الإجمالي', c.buyerNet + c.amrTotal + c.moazTotal]);
  return rows;
};

const exportCurrentExcel = () => { const chk = getCurrentCheck(); if (!chk) { alert('لا يوجد شيك محدد'); return; } const c = computeCheck(chk); const wb = XLSX.utils.book_new(); const ws = XLSX.utils.aoa_to_sheet(buildExcelRows(chk, c)); ws['!cols'] = [{wch:22}, {wch:22}, {wch:5}, {wch:28}, {wch:18}, {wch:12}, {wch:22}, {wch:18}]; XLSX.utils.book_append_sheet(wb, ws, 'تقرير الشيك'); XLSX.writeFile(wb, `تقرير_شيك_${chk.checkNumber||'بدون_رقم'}_${chk.workMonth}.xlsx`); };
const exportAllExcel = () => { const checks = safeArray(state.checks); if (checks.length === 0) { alert('لا توجد بيانات'); return; } const wb = XLSX.utils.book_new(); const summaryRows = [['شهر العمل','رقم الشيك','تاريخ الصرف','المبلغ','التوريد','13%','1%','الاستلام','التحصيل']]; checks.forEach(chk => { const c = computeCheck(chk); summaryRows.push([workMonthLabel(chk.workMonth), chk.checkNumber||'-', chk.checkDate||'-', c.checkAmount, c.supply, c.pct13, c.pct1, chk.received?'نعم':'لا', chk.collectionDate||'-']); }); const ws1 = XLSX.utils.aoa_to_sheet(summaryRows); ws1['!cols'] = [{wch:18},{wch:14},{wch:14},{wch:16},{wch:16},{wch:12},{wch:12},{wch:12},{wch:16}]; XLSX.utils.book_append_sheet(wb, ws1, 'ملخص الشيكات'); const allC = checks.map(c => computeCheck(c)); const cumRows = [['الإجماليات التراكمية'],['عدد الشيكات', checks.length],['إجمالي قيمة الشيكات', allC.reduce((s,c)=>s+c.checkAmount,0)],['إجمالي 22.5%', allC.reduce((s,c)=>s+c.tax225,0)],['إجمالي 1%', allC.reduce((s,c)=>s+c.pct1,0)],['المتبقي من 22.5%', round2(Math.max(0, allC.reduce((s,c)=>s+c.tax225,0) - allC.reduce((s,c)=>s+c.pct1,0)))]]; const ws2 = XLSX.utils.aoa_to_sheet(cumRows); ws2['!cols'] = [{wch:35},{wch:20}]; XLSX.utils.book_append_sheet(wb, ws2, 'الإجماليات'); XLSX.writeFile(wb, `كل_الشيكات_${new Date().toISOString().slice(0,10)}.xlsx`); };

const exportJSON = () => {
  const existingRaw = localStorage.getItem(STORAGE_KEY);
  const existingData = existingRaw ? JSON.parse(existingRaw) : {};
  const data = { version: APP_CONFIG.VERSION, exportedAt: new Date().toISOString(), state: { shared: Store.shared, financing: existingData.financing || {}, supply: { checks: state.checks, transfers: state.transfers, nextCheckNumber: state.nextCheckNumber || 1, currentCheckId: state.currentCheckId || null, currentPage: state.currentPage || 'checks' } } };
  const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a'); a.href = url; a.download = `unified_backup_supply_v${APP_CONFIG.VERSION}_${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url);
};
...
const importJSON = (e) => {
  ...
  reader.onload = (ev) => {
    try {
      ...
      if (!data.state) { alert('⛔ ملف JSON غير صالح: البنية الأساسية للبيانات مفقودة'); return; }
      ...
      if (data.state.shared) { Store.shared.customers = Array.isArray(data.state.shared.customers) ? data.state.shared.customers : []; Store.shared.funders = Array.isArray(data.state.shared.funders) ? data.state.shared.funders : []; }
      state.checks = Array.isArray(supplyData.checks) ? supplyData.checks.map(chk => ({ ...chk, allocations: Array.isArray(chk.allocations || chk.supplyOrders) ? (chk.allocations || chk.supplyOrders) : [], expenses: Array.isArray(chk.expenses) ? chk.expenses : [], checkAmount: parseNum(chk.checkAmount || chk.amount), taxablePurchase: parseNum(chk.taxablePurchase), vat: parseNum(chk.vat) })) : [];
      state.currentCheckId = supplyData.currentCheckId || null; state.currentPage = supplyData.currentPage || 'checks'; state.nextCheckNumber = typeof supplyData.nextCheckNumber === 'number' ? supplyData.nextCheckNumber : 1;
      const transfersData = supplyData.transfers || {};
      state.transfers = { transactions: Array.isArray(transfersData.transactions) ? transfersData.transactions : [], purchases: Array.isArray(transfersData.purchases) ? transfersData.purchases : [] };
      saveState(); switchPage(state.currentPage); alert('✅ تم استيراد البيانات بنجاح وبأمان، وتم دمجها مع النواة المشتركة');
...
const bindEvents = () => {
  ...
  bind('btn-clear-all', 'click', () => {
    ...
    state.checks = []; state.transfers = { transactions: [], purchases: [] }; state.currentCheckId = null; state.nextCheckNumber = 1; state.currentPage = 'checks';
    Store.shared.customers = []; Store.shared.funders = [];
    saveState(); renderAll(); alert('✅ تم حذف جميع البيانات بنجاح وإعادة تعيين النظام.');
  });
